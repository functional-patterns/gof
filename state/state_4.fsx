...
let ret (x : int) : State = fun s -> (x, s)

// (>>=) :: State s a -> (a -> State s b) -> State s b  

let (>>=) (h : State) (f : int -> State) : State =
    fun s ->
        let (a, newState) = h s
        let g = f a
        g newState
...
//let inc (x : int) : State = fun state -> (x, snd state)
let inc (x : int) : State = (fun stack -> (x + 1, stack))

 

let wtf = (ret 5) >>= inc >>= inc
